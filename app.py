    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if exercise_type == 'QCM':
                ...
                left_images = request.files.getlist('left_image[]')
                right_images = request.files.getlist('right_image[]')
                ...
                for i in range(len(left_types)):
                    ...
                    left_content = left_contents[i] if left_type == 'text' else ''
                    right_content = right_contents[i] if right_type == 'text' else ''
                    
                    # Gérer les images si présentes
                    if left_type == 'image' and i < len(left_images) and left_images[i].filename:
                        image_file = left_images[i]
                        filename = secure_filename(image_file.filename)
                        image_path = os.path.join('uploads', filename)
                        full_path = os.path.join(app.static_folder, 'uploads', filename)
                        image_file.save(full_path)
                    ...
                    if right_type == 'image' and i < len(right_images) and right_images[i].filename:
                        image_file = right_images[i]
                        filename = secure_filename(image_file.filename)
                        image_path = os.path.join('uploads', filename)
                        full_path = os.path.join(app.static_folder, 'uploads', filename)
                        image_file.save(full_path)
...
def view_exercise(exercise_id):
    ...
    try:
        ...
        # Charger les paires pour les exercices de type PAIR_MATCH
        elif exercise.exercise_type == 'PAIR_MATCH':
            pair_matches = exercise.pair_matches
            logger.debug(f"Nombre de paires: {len(pair_matches)}")
...
def submit_exercise(exercise_id):
    try:
        ...
        if exercise.exercise_type == 'QCM':
            ...
            if not matches:
                flash('Aucune correspondance soumise.', 'error')
                return redirect(url_for('view_exercise', exercise_id=exercise_id))
            ...
            try:
                ...
                for match in matches:
                    if not isinstance(match, dict) or 'left_id' not in match or 'right_id' not in match:
        ...
        # Calculate the score final en pourcentage
